# tests/test_utils/test_date_extractor.py
import pytest
...
class TestExtractListingDate:
    ...
    def test_extract_listing_date_realtor(self, sample_soup):
        ...
        date = DateExtractor.extract_listing_date(sample_soup, "Realtor.com")
    ...
    def test_extract_listing_date_land_and_farm(self, sample_soup):
        ...
        date = DateExtractor.extract_listing_date(sample_soup, "Land and Farm")
        assert date == "2023-01-15"  # Uses first date found
    
    def test_extract_listing_date_no_date(self):
        """Test handling when no date is found."""
        soup = BeautifulSoup("<html><div>No date here</div></html>", 'html.parser')
    ...
    def test_extract_listing_date_with_time_element(self, sample_soup):
        ...
        for tag in sample_soup.find_all(["div", "span"]):
            tag.decompose()
        date = DateExtractor.extract_listing_date(sample_soup, "Any Platform")
...
class TestParseDateString:
    ...
    def test_parse_date_string_with_text(self):
        ...
        date = DateExtractor.parse_date_string("Listed on January 15, 2023")
        assert date == "2023-01-15"
    ...
    def test_parse_date_string_invalid(self):
        ...
        assert DateExtractor.parse_date_string("13/45/2023") is None  # Invalid date
...
class TestIsRecentListing:
    ...
    def test_is_recent_listing_old(self):
        ...
        forty_five_days_ago = (datetime.now() - timedelta(days=45)).strftime('%Y-%m-%d')
        assert DateExtractor.is_recent_listing(forty_five_days_ago) is False
    ...
    def test_is_recent_listing_custom_days(self):
        ...
        assert DateExtractor.is_recent_listing(ten_days_ago, days=15) is True
        
        # 20 days ago with 15-day threshold (should not be recent)
        twenty_days_ago = (datetime.now() - timedelta(days=20)).strftime('%Y-%m-%d')
        assert DateExtractor.is_recent_listing(twenty_days_ago, days=15) is False
...
class TestExtractDateFromText:
    ...
    def test_extract_date_from_text_various_formats(self):
        ...
        for format_str in formats:
            date = DateExtractor.extract_date_from_text(f"Date: {format_str}")
...
# Test the backwards compatibility functions
class TestBackwardsCompatibility:
    def test_extract_listing_date_function(self):
        """Test that the standalone function works."""
        from new_england_listings.utils.dates import extract_listing_date
        soup = BeautifulSoup("<div class='listing-date'>January 15, 2023</div>", 'html.parser')
        date = extract_listing_date(soup, "Any Platform")
        assert date == "2023-01-15"
    
    def test_parse_date_string_function(self):
        """Test that the standalone function works."""
        from new_england_listings.utils.dates import parse_date_string
        date = parse_date_string("January 15, 2023")
        assert date == "2023-01-15"
    
    def test_is_recent_listing_function(self):
        """Test that the standalone function works."""
        from new_england_listings.utils.dates import is_recent_listing
        today = datetime.now().strftime('%Y-%m-%d')
